...
export async function daySummary(params: { from?: Date; to?: Date }) {
...
export async function salesByPeriod(params: { from: Date; to: Date }) {
...
export async function topProducts(params: { from: Date; to: Date; limit?: number }) {
  ...
  const map = new Map<string, { productId: string; nome: string; categoria: string; quantidade: number; total: number }>();
