...
function assertIdentification(params) {
    if (params.tipo === 'MESA') {
        if (!params.tableId)
            throw new HttpError(422, 'ORDER_IDENTIFICATION_REQUIRED');
        return;
    }
    if (!params.nomeCliente?.trim())
        throw new HttpError(422, 'ORDER_IDENTIFICATION_REQUIRED');
}
export async function openOrder(params) {
    const cash = await getOpenCashRegister();
    if (!cash)
        throw new HttpError(409, 'CASH_REGISTER_MUST_BE_OPEN');
    ...
    if (params.tipoIdentificacao === 'MESA' && params.tableId) {
        ...
        if (!table)
            throw new HttpError(404, 'TABLE_NOT_FOUND');
        if (table.status !== 'LIVRE')
            throw new HttpError(409, 'TABLE_NOT_AVAILABLE');
    }
...
export async function convertOrderIdentification(params) {
    ...
    if (order.status === 'FECHADO')
        throw new HttpError(409, 'ORDER_ALREADY_CLOSED');
...
export async function addOrderItem(params) {
    ...
    if (params.quantidade <= 0)
        throw new HttpError(422, 'INVALID_QUANTITY');
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (product.controlaEstoque) {
            ...
            if (!stock)
                throw new HttpError(409, 'STOCK_NOT_CONFIGURED');
            if (stock.quantidadeAtual < params.quantidade)
...
export async function cancelOrderItem(params) {
    ...
    if (item.canceladoEm)
        throw new HttpError(409, 'ITEM_ALREADY_CANCELED');
    if (item.order.status === 'FECHADO')
        throw new HttpError(409, 'ORDER_ALREADY_CLOSED');
    return prisma.$transaction(async (tx) => {
...
export async function addPayment(params) {
    ...
    if (order.status === 'FECHADO')
        throw new HttpError(409, 'ORDER_ALREADY_CLOSED');
    if (params.valor <= 0)
        throw new HttpError(422, 'INVALID_AMOUNT');
...
export async function closeOrder(params) {
    ...
    if (order.status === 'FECHADO')
        throw new HttpError(409, 'ORDER_ALREADY_CLOSED');
    if (order.itens.filter((i) => !i.canceladoEm).length === 0)
        throw new HttpError(409, 'ORDER_HAS_NO_ITEMS');
    const paid = order.pagamentos.reduce((sum, p) => sum + p.valor, 0);
    if (paid < order.valorTotal)
        throw new HttpError(409, 'INSUFFICIENT_PAYMENT');
    return prisma.$transaction(async (tx) => {
        const updated = await tx.order.update({
            where: { id: params.orderId },
